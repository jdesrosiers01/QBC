...
void static QuebecoinMiner(CWallet *pwallet)
{
    printf("QuebecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("quebecoin-miner");
    ...
    try { loop {
        ...
        printf("Running QuebecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("QuebecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&QuebecoinMiner, pwallet));
}
